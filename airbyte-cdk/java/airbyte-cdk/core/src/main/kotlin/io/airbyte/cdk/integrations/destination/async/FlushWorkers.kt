 ...
 * [DestinationFlushFunction.getOptimalBatchSizeBytes] batch from the in-memory stream and
...
class FlushWorkers(
    ...
    private fun retrieveWork() {
        try {
            ...
            val threadPoolExecutor = (workerPool as InstrumentedExecutorService).target as ThreadPoolExecutor
    ...
    fun close() {
        ...
        while (true) {
                    ...
                    .collect(
                        Collectors.toMap(
                            ...
                            { desc: StreamDescriptor? ->
                                bufferDequeue.getQueueSizeInRecords(
                                    desc!!,
                                ).orElseThrow()
                            },
                        ),
            ...
            val workerInfo = java.lang.StringBuilder().append("REMAINING_BUFFERS_INFO").append(System.lineSeparator())
            ...
            Runnable {
                ...
                try {
                    ...
                    bufferDequeue.take(desc, destinationFlushFunction.optimalBatchSizeBytes).use { batch ->
                        ...
                        destinationFlushFunction.flush(
                            ...
                            batch.getData().stream()
                                .map(StreamAwareQueue.MessageWithMeta::message),
                        )
                    ...
                    logger.info {
                        "Flush Worker (${humanReadableFlushWorkerId(
                            flushWorkerId,
                        )}) -- Worker finished flushing. Current queue size: ${bufferDequeue.getQueueSizeInRecords(desc).orElse(-1)}"
                    }
                } catch (e: Exception) {
